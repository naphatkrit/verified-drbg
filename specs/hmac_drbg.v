...
Definition _p_rng : ident := 112%positive.
Definition _interval : ident := 110%positive.
...
Definition _mbedtls_hmac_drbg_set_reseed_interval : ident := 111%positive.
...
Definition _mbedtls_hmac_drbg_set_prediction_resistance : ident := 108%positive.
Definition _custom : ident := 104%positive.
...
Definition _mbedtls_hmac_drbg_free : ident := 119%positive.
...
Definition _mbedtls_hmac_drbg_set_entropy_len : ident := 109%positive.
...
Definition _out_len : ident := 113%positive.
Definition _seedlen : ident := 102%positive.
Definition _mbedtls_md_info_t : ident := 7%positive.
Definition _mbedtls_hmac_drbg_reseed : ident := 103%positive.
Definition _mbedtls_hmac_drbg_seed_buf : ident := 99%positive.
Definition ___builtin_fabs : ident := 21%positive.
Definition _p_entropy : ident := 18%positive.
Definition _seed : ident := 101%positive.
Definition _mbedtls_hmac_drbg_init : ident := 89%positive.
Definition ___i64_shr : ident := 45%positive.
Definition _mbedtls_hmac_drbg_seed : ident := 106%positive.
...
Definition _out : ident := 115%positive.
Definition _len : ident := 100%positive.
Definition _left : ident := 114%positive.
...
Definition _mbedtls_hmac_drbg_random_with_add : ident := 117%positive.
Definition _md_size : ident := 105%positive.
Definition _ret : ident := 70%positive.
Definition ___builtin_annot_intval : ident := 24%positive.
Definition _mbedtls_hmac_drbg_update : ident := 96%positive.
...
Definition _resistance : ident := 107%positive.
...
Definition _mbedtls_hmac_drbg_random : ident := 118%positive.
Definition _data_len : ident := 98%positive.
...
Definition _use_len : ident := 116%positive.
Definition _data : ident := 97%positive.
...
Definition f_test_md_get_size := {|
  ...
  fn_temps := ((_ret, tuchar) :: (121%positive, tuchar) :: nil);
  fn_body :=
(Ssequence
  ...
  (Sset _ret (Ecast (Etempvar 121%positive tuchar) tuchar)))
|}.
...
Definition f_mbedtls_zeroize := {|
  ...
  fn_temps := ((_p, (tptr (tvolatile tuchar))) ::
               (123%positive, (tptr (tvolatile tuchar))) ::
               (122%positive, tuint) :: nil);
  fn_body :=
(Ssequence
  ...
  (Sloop
    (Ssequence
      (Ssequence
        (Ssequence
          (Sset 122%positive (Etempvar _n tuint))
          (Sset _n
            (Ebinop Osub (Etempvar 122%positive tuint)
              (Econst_int (Int.repr 1) tint) tuint)))
        (Sifthenelse (Etempvar 122%positive tuint) Sskip Sbreak))
      (Ssequence
        (Ssequence
          (Sset 123%positive (Etempvar _p (tptr (tvolatile tuchar))))
          (Sset _p
            (Ebinop Oadd (Etempvar 123%positive (tptr (tvolatile tuchar)))
        ...
        (Sbuiltin None (EF_vstore Mint8unsigned)
          ...
          ((Eaddrof
             (Ederef (Etempvar 123%positive (tptr (tvolatile tuchar)))
