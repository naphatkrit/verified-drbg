...
Fixpoint DRBG_generate_check (state_handle: DRBG_state_handle) (internal_states: list (string * string * string * string)) (returned_bits: string) :=
  match internal_states with
    ...
    | (key, v, additional_input, entropy_input_reseed)::[] =>
      let test_HMAC256_DRBG_reseed_function := HMAC256_DRBG_reseed_function (get_entropy_input_dummy entropy_input_reseed) 0 0 256 in
      ...
      match test_HMAC256_DRBG_generate_function state_handle 128 256 true additional_input with
...
Definition DRBG_check (entropy_input nonce key value personalization_string: string) (internal_states: list (string * string * string * string)) (returned_bits: string) :=
  ...
  match HMAC256_DRBG_instantiate_function (get_entropy_input_dummy entropy_input) 0 0 (get_nonce_dummy nonce) 256 256 true 256 true personalization_string with
    | instantiate_success ((value', key', x), y, z) => listZ_eq value value' = true /\ listZ_eq key key' = true /\ DRBG_generate_check ((value', key', x), y, z) internal_states returned_bits
    | _ => False
  end.
